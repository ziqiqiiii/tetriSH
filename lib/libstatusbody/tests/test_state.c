...
void	test_state_encode_contains_all_required_keys(void)
{
	...
	make_state(&st);
	assert(sb_state_encode(&st, out, sizeof(out)) > 0);
...
void	test_state_round_trip_full_frame(void)
{
	...
	n = sb_state_encode(&in, out, sizeof(out));
	assert(n > 0);
	memset(&back, 0, sizeof(back));
	assert(sb_state_decode(out, (size_t)n, &back) == 0);
...
void	test_state_board_cells_preserve_type_and_color(void)
{
	...
	assert(row0 != NULL);
	row0 += 6;
	assert(strchr(row0, '\n') - row0 == SB_BOARD_COLS * 2); // 20 hex chars
	memset(&back, 0, sizeof(back));
	assert(sb_state_decode(out, (size_t)n, &back) == 0);
...
void	test_state_encode_rejects_out_of_range_fields(void)
{
	...
	make_state(&st);
	st.charge = SB_CHARGE_MAX + 1;
	assert(sb_state_encode(&st, out, sizeof(out)) == -1);
	assert(errno == EINVAL);
	make_state(&st);
	st.clearing_count = SB_CLEARING_MAX + 1;
	assert(sb_state_encode(&st, out, sizeof(out)) == -1);
	assert(errno == EINVAL);
	make_state(&st);
...
void	test_state_encode_small_cap_returns_erange(void)
{
	...
	assert(sb_state_encode(&st, out, 32) == -1);
	assert(errno == ERANGE);
...
void	test_state_decode_rejects_missing_seq(void)
{
	...
	build_body(body, g_head + 7, SB_BOARD_ROWS, NULL); // skip "seq 42\n"
	assert(sb_state_decode(body, strlen(body), &st) == -1);
...
void	test_state_decode_rejects_malformed_board_row(void)
{
	...
	build_body(body, g_head, SB_BOARD_ROWS - 1, "000000000000000000\n");
	assert(sb_state_decode(body, strlen(body), &st) == -1);
	assert(errno == EBADMSG);
	// last row is not hex
	build_body(body, g_head, SB_BOARD_ROWS - 1, "zzzzzzzzzzzzzzzzzzzz\n");
	assert(sb_state_decode(body, strlen(body), &st) == -1);
	assert(errno == EBADMSG);
	// only 19 rows
	build_body(body, g_head, SB_BOARD_ROWS - 1, NULL);
	assert(sb_state_decode(body, strlen(body), &st) == -1);
	assert(errno == EBADMSG);
...
void	test_state_decode_rejects_unknown_key_and_trailing_junk(void)
{
	...
	strcat(head, g_head);
	build_body(body, head, SB_BOARD_ROWS, NULL);
	assert(sb_state_decode(body, strlen(body), &st) == -1);
	assert(errno == EBADMSG);
	build_body(body, g_head, SB_BOARD_ROWS, "extra\n"); // junk after board
	assert(sb_state_decode(body, strlen(body), &st) == -1);
	assert(errno == EBADMSG);
