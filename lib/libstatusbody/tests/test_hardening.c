...
void	test_all_decoders_reject_empty_buffer(void)
{
	...
	assert(sb_state_decode("", 0, &st) == -1);
	assert(errno == EBADMSG);
	assert(sb_profile_decode("", 0, &pr) == -1);
	assert(errno == EBADMSG);
	assert(sb_rooms_decode("", 0, rooms, 2, &count) == 0);
	assert(count == 0);
	assert(sb_leaderboard_decode("", 0, lb, 2, &count) == 0);
	assert(count == 0);
...
void	test_all_codecs_reject_null_arguments(void)
{
	...
	assert(sb_state_encode(NULL, out, sizeof(out)) == -1);
	assert(errno == EINVAL);
	assert(sb_state_encode(&st, NULL, 64) == -1);
	assert(errno == EINVAL);
	assert(sb_state_decode(NULL, 4, &st) == -1);
	assert(errno == EINVAL);
	assert(sb_state_decode("x", 1, NULL) == -1);
	assert(errno == EINVAL);
	assert(sb_rooms_encode(rooms, 2, NULL, 64) == -1);
	assert(errno == EINVAL);
	assert(sb_rooms_decode("x", 1, NULL, 2, &count) == -1);
	assert(errno == EINVAL);
	assert(sb_rooms_decode("x", 1, rooms, 2, NULL) == -1);
	assert(errno == EINVAL);
	assert(sb_profile_encode(NULL, out, sizeof(out)) == -1);
	assert(errno == EINVAL);
	assert(sb_profile_decode("x", 1, NULL) == -1);
	assert(errno == EINVAL);
	assert(sb_leaderboard_encode(lb, 2, NULL, 64) == -1);
	assert(errno == EINVAL);
	assert(sb_leaderboard_decode("x", 1, NULL, 2, &count) == -1);
	assert(errno == EINVAL);
...
void	test_worst_case_encodes_fit_frame_cap(void)
{
	...
	st.seq = UINT64_MAX;
	st.score = UINT64_MAX;
	st.charge = SB_CHARGE_MAX;
	st.clearing_count = SB_CLEARING_MAX;
	row = 0;
	while (row < SB_BOARD_ROWS)
	{
		...
		while (col < SB_BOARD_COLS)
		{
			...
			st.cells[row][col].color = SB_COLOR_MAX;
			col++;
		}
		row++;
	}
	n = sb_state_encode(&st, g_out, sizeof(g_out));
	assert(n > 0 && n < 65536);
	i = 0;
	while (i < 99)
	{
	...
	pr.owned_theme_count = SB_OWNED_MAX;
	i = 0;
	while (i < SB_OWNED_MAX)
	{
		pr.owned_characters[i] = UINT32_MAX;
		pr.owned_themes[i] = UINT32_MAX;
		i++;
	}
	n = sb_profile_encode(&pr, g_out, sizeof(g_out));
	assert(n > 0 && n < 65536);
	i = 0;
	while (i < 10)
	{
	...
	n = sb_leaderboard_encode(lb, 10, g_out, sizeof(g_out));
	assert(n > 0 && n < 65536);
...
void	test_decode_is_bounded_on_hostile_input(void)
{
	...
	assert(sb_state_decode(g_big, 65536, &st) == -1);
	assert(sb_profile_decode(g_big, 65536, &pr) == -1);
	assert(sb_rooms_decode(g_big, 65536, rooms, 4, &count) == -1);
	assert(sb_leaderboard_decode(g_big, 65536, lb, 4, &count) == -1);
	memset(g_big, 0, 64); // a line of NULs
	assert(sb_state_decode(g_big, 64, &st) == -1);
	assert(sb_profile_decode(g_big, 64, &pr) == -1);
	strcpy(g_big, "seq 42"); // valid-ish start, no trailing newline
	assert(sb_state_decode(g_big, 6, &st) == -1);
