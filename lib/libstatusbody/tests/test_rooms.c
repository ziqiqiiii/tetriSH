...
void	test_rooms_encode_one_line_per_row(void)
{
	...
	make_row(&rows[0], "D-01", BODY_MODE_DOUBLE, BODY_ROOM_WAITING, "alice");
	make_row(&rows[1], "BR-02", BODY_MODE_BATTLE_ROYALE, BODY_ROOM_READY, "bob");
	rows[1].players = 5;
	rows[1].slot_count = 8;
	make_row(&rows[2], "S-03", BODY_MODE_SINGLE, BODY_ROOM_WAITING, "cara");
	rows[2].slot_count = 1;
	n = body_rooms_encode(rows, 3, out, sizeof(out));
	assert(n > 0);
	assert(strcmp(out,
...
void	test_rooms_round_trip(void)
{
	...
	make_row(&in[0], "S-01", BODY_MODE_SINGLE, BODY_ROOM_WAITING, "alice");
	make_row(&in[1], "D-01", BODY_MODE_DOUBLE, BODY_ROOM_READY, "bob");
	make_row(&in[2], "BR-01", BODY_MODE_BATTLE_ROYALE, BODY_ROOM_IN_GAME, "cara");
	make_row(&in[3], "D-02", BODY_MODE_DOUBLE, BODY_ROOM_FINISHED, "dan");
	n = body_rooms_encode(in, 4, out, sizeof(out));
	assert(n > 0);
	memset(back, 0, sizeof(back));
	assert(body_rooms_decode(out, (size_t)n, back, 4, &count) == 0);
	assert(count == 4);
...
void	test_rooms_empty_list_round_trips(void)
{
	...
	assert(body_rooms_encode(NULL, 0, out, sizeof(out)) == 0);
	assert(out[0] == '\0');
	count = 99;
	assert(body_rooms_decode(out, 0, back, 4, &count) == 0);
	assert(count == 0);
...
void	test_rooms_decode_rejects_bad_mode_or_status_token(void)
{
	...
	assert(body_rooms_decode(bad_mode, strlen(bad_mode), back, 4,
			&count) == -1);
	assert(errno == EBADMSG);
	bad_status = "D-01 DOUBLE 1/2 LOBBY alice\n";
	assert(body_rooms_decode(bad_status, strlen(bad_status), back, 4,
			&count) == -1);
	assert(errno == EBADMSG);
...
void	test_rooms_decode_more_rows_than_cap_fails(void)
{
	...
	make_row(&in[0], "D-01", BODY_MODE_DOUBLE, BODY_ROOM_WAITING, "alice");
	make_row(&in[1], "D-02", BODY_MODE_DOUBLE, BODY_ROOM_WAITING, "bob");
	make_row(&in[2], "D-03", BODY_MODE_DOUBLE, BODY_ROOM_WAITING, "cara");
	n = body_rooms_encode(in, 3, out, sizeof(out));
	assert(n > 0);
	assert(body_rooms_decode(out, (size_t)n, back, 2, &count) == -1);
	assert(errno == ERANGE);
...
void	test_rooms_decode_rejects_overlong_name_or_owner(void)
{
	...
	// 16+ chars, so it cannot fit BODY_NAME_MAX with its NUL
	long_name = "D-0123456789ABCDEF DOUBLE 1/2 WAITING alice\n";
	assert(body_rooms_decode(long_name, strlen(long_name), back, 2,
			&count) == -1);
	assert(errno == EBADMSG);
	long_owner = "D-01 DOUBLE 1/2 WAITING "
		"a-thirty-two-plus-character-owner-name\n";
	assert(body_rooms_decode(long_owner, strlen(long_owner), back, 2,
			&count) == -1);
	assert(errno == EBADMSG);
