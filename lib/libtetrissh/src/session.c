...
ssize_t	session_send(t_session *sess, const void *buf, size_t len)
{
	...
	if (gcm_encrypt(sess, buf, len, &frame, &frame_len) != 0)
		return (-1);
	if (sessionio_write_u32(sess->fd, frame_len) != 0
		|| sessionio_write_exact(sess->fd, frame, frame_len) != 0)
	{
		...
		size_t plain_len, unsigned char **frame, uint32_t *frame_len)
{
	...
	if (plain_len > TETRISSH_MAX_PLAINTEXT || plain_len > UINT32_MAX)
		return (-1);
	*frame_len = (uint32_t)(SESSIONIO_FRAME_OVERHEAD + plain_len);
	token = malloc(*frame_len);
	if (token == NULL)
		return (-1);
	if (RAND_bytes(token, SESSIONIO_GCM_NONCE_LEN) != 1)
		return (free(token), -1);
	...
	if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1
		...
		|| EVP_EncryptUpdate(ctx, token + SESSIONIO_FRAME_OVERHEAD, &out_len,
			plain, (int)plain_len) != 1
		|| EVP_EncryptFinal_ex(ctx, token + SESSIONIO_FRAME_OVERHEAD + out_len,
			&final_len) != 1
		|| EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG,
			SESSIONIO_GCM_TAG_LEN, token + SESSIONIO_GCM_NONCE_LEN) != 1)
	{
		...
		uint32_t frame_len, unsigned char **plain, size_t *plain_len)
{
	...
	if (frame_len < SESSIONIO_FRAME_OVERHEAD)
		return (-1);
	*plain_len = (size_t)frame_len - SESSIONIO_FRAME_OVERHEAD;
	...
	if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1
		...
		|| EVP_DecryptUpdate(ctx, out, &out_len,
			frame + SESSIONIO_FRAME_OVERHEAD, (int)*plain_len) != 1
		|| EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG,
			SESSIONIO_GCM_TAG_LEN, (void *)(frame + SESSIONIO_GCM_NONCE_LEN)) != 1)
	{
		EVP_CIPHER_CTX_free(ctx);
