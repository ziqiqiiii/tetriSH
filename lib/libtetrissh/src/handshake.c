		...
		const char *cert_path, const char *key_path)
{
	...
	if (fd < 0 || cert_path == NULL || key_path == NULL)
		goto cleanup;
	if (sessionio_read_exact(fd, client_nonce, sizeof(client_nonce)) != SESSIONIO_IO_OK)
		goto cleanup;
	...
	if (sessionio_write_u32(fd, cert_len) != 0
		...
		goto cleanup;
	if (sessionio_read_u32(fd, &wrapped_len) != SESSIONIO_IO_OK
		|| wrapped_len != (uint32_t)key_size)
		goto cleanup;
	wrapped = malloc(wrapped_len);
	if (wrapped == NULL)
		goto cleanup;
	if (sessionio_read_exact(fd, wrapped, wrapped_len) != SESSIONIO_IO_OK)
		goto cleanup;
...
int	session_handshake_client(int fd, t_session *sess, const char *ca_path)
{
	...
	if (sessionio_write_exact(fd, client_nonce, sizeof(client_nonce)) != 0)
		goto cleanup;
	if (sessionio_read_u32(fd, &cert_len) != SESSIONIO_IO_OK || cert_len == 0
		|| cert_len > SESSIONIO_MAX_CERT_LEN)
		goto cleanup;
	cert_bytes = malloc(cert_len);
	if (cert_bytes == NULL)
		goto cleanup;
	if (sessionio_read_exact(fd, cert_bytes, cert_len) != SESSIONIO_IO_OK)
		goto cleanup;
	...
	if (key_size <= 0)
		goto cleanup;
	if (sessionio_read_u32(fd, &sig_len) != SESSIONIO_IO_OK
		|| sig_len != (uint32_t)key_size)
		goto cleanup;
	sig = malloc(sig_len);
	if (sig == NULL)
		goto cleanup;
	if (sessionio_read_exact(fd, sig, sig_len) != SESSIONIO_IO_OK)
		goto cleanup;
