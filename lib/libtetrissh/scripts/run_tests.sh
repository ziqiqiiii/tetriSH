...
#   e.g.  make test FILTER=io
#         make test FILTER=session_security
#
...
run_unit() {
  ...
  for t in "$@"; do
    ...
    echo "$output" | awk -v grn="$GRN" -v red="$RED" -v rst="$RST" -v cyn="$CYN" -F: '
      /^-{3,}/ || /^OK$/ || /^FAIL$/ { next }
      # Plain "PASS <name>" / "FAIL <name>" output (libtetrissh tests).
      /^PASS / { printf "  %-50s %sPASS%s\n", substr($0, 6), grn, rst; next }
      /^FAIL / { printf "  %-50s %sFAIL%s\n", substr($0, 6), red, rst; next }
      # Unity-style "file:line:name:PASS|FAIL[:detail]" output.
