...
| Suite | Proves |
|---|---|
| `test_detach` | A daemon that reports ready exits its parent `0`; one that dies during boot exits it non-zero; the detached process is not a session leader; `cd_stderr_redirect` moves the stream and refuses an unusable path |
| `test_pidfile` | Claim writes the pid and creates parents; a second process loses the lock with `EWOULDBLOCK`; release hands it on; a probe reads a live holder's pid, and reads a crash-left pidfile as *nothing running*; `cd_pid_wait` times out on a live holder and returns once it is gone |
| `test_paths` | `cd_mkdir_p` builds a whole chain and is idempotent; a regular file in the way is `ENOTDIR`, not success; `cd_mkdir_parent` ignores the leaf |
...
|---|---|
| `int cd_detach(int *ready_fd)` | Double-fork into the background. Returns `0` in the daemon only; the originating process blocks inside this call and exits `0` or `1` according to what happens next. |
| `void cd_ready(int ready_fd)` | Report a successful boot, releasing the process that launched us. |
| `int cd_stderr_redirect(const char *path)` | Point `stderr` at a file, once there is nothing left to fail. |

### The pidfile — daemon side

| Function | Description |
|---|---|
| `void cd_pid_blank(t_pidfile *pf)` | Put a zeroed pidfile into the "nothing is held" state. Call before anything that can fail. |
| `int cd_pid_claim(t_pidfile *pf, const char *path)` | Take the lock, then write the pid. `-1` with `errno == EWOULDBLOCK` means another instance is running. |
| `void cd_pid_release(t_pidfile *pf)` | Release the lock. Safe on a pidfile that was never claimed. |

### The pidfile — `tetrisctl` side

| Function | Description |
|---|---|
| `int cd_pid_read(const char *path, pid_t *out)` | What the file says, regardless of who holds it. |
| `int cd_pid_probe(const char *path, pid_t *out)` | `1` running (with the pid), `0` not running, `-1` on error. |
| `int cd_pid_wait(const char *path, int timeout_ms)` | Block until the lock comes free. `-1` with `errno == ETIMEDOUT` on timeout; a negative timeout waits indefinitely. |

### Paths

| Function | Description |
|---|---|
| `int cd_mkdir_p(const char *path)` | Create a directory and every missing directory above it. |
| `int cd_mkdir_parent(const char *path)` | Create the directories a file path needs, ignoring the file itself. |
...
`cd_detach` hands back the write end of a pipe the launching process is
...
`cd_detach` takes stdin and stdout to `/dev/null` and leaves `stderr` alone.
Everything between `cd_detach` and `cd_ready` is boot, and a boot that fails
has to say why somewhere a person is looking. `cd_stderr_redirect` moves the
...
- **Claim after `cd_detach`, never before.** The pid written has to be the
  detached process's, and the lock has to be held by the process that will
  still be here to hold it.
- **`cd_detach` belongs in `main()` alone.** Putting it behind a `*_start()`
  function would make every in-process test suite fork the moment it booted a
  daemon.

`cd_pid_release` leaves the file on disk. A pidfile whose lock is free already
...
├── src/
│   ├── detach.c            # cd_detach, cd_ready, cd_stderr_redirect
│   ├── pidfile.c           # cd_pid_blank, cd_pid_claim, cd_pid_release
│   ├── probe.c             # cd_pid_read, cd_pid_probe, cd_pid_wait
│   └── paths.c             # cd_mkdir_p, cd_mkdir_parent
├── tests/
