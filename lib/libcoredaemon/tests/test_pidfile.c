...
static void	test_probe_reports_nothing_for_a_missing_pidfile(void)
{
	...
	pid = 4242;
	assert(cd_pid_probe(path, &pid) == 0);
	assert(pid == 0);
	fx_rmtree(dir);
...
static void	test_probe_reports_the_pid_of_a_live_holder(void)
{
	...
	assert(fx_hold(&h, path) == 0);
	pid = 0;
	assert(cd_pid_probe(path, &pid) == 1);
	assert(pid == h.pid);
	assert(cd_pid_wait(path, 40) == -1 && errno == ETIMEDOUT);
	fx_stop(&h);
	assert(cd_pid_wait(path, 1000) == 0);
	fx_rmtree(dir);
...
static void	test_read_parses_an_unheld_pidfile(void)
{
	...
	close(fd);
	pid = 0;
	assert(cd_pid_read(path, &pid) == 0);
	assert(pid == 1234);
	fd = open(path, O_WRONLY | O_TRUNC);
	assert(fd >= 0 && write(fd, "not-a-pid\n", 10) == 10);
	close(fd);
	assert(cd_pid_read(path, &pid) == -1);
	fx_rmtree(dir);
...
static void	test_release_is_safe_on_a_blank_pidfile(void)
{
	...
	cd_pid_release(&pf);
	cd_pid_release(NULL);
	assert(cd_pid_claim(NULL, "x") == -1);
	assert(cd_pid_claim(&pf, NULL) == -1);
	assert(cd_pid_read(NULL, NULL) == -1);
	assert(cd_pid_probe(NULL, NULL) == -1);
