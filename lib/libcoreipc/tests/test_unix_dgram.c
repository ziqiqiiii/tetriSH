...
void	test_datagram_roundtrip(void)
{
	...
	tmp_path(path, sizeof(path), "logd.sock");
	rx = unixsock_dgram_bind(path, 0600);
	assert(rx >= 0);
	tx = unixsock_dgram_open(path);
	assert(tx >= 0);
	assert(unixsock_dgram_send_nonblock(tx, "record-one", 10) == 0);
	assert(unixsock_dgram_send_nonblock(tx, "record-two", 10) == 0);
	memset(buf, 0, sizeof(buf));
	assert(unixsock_dgram_recv(rx, buf, sizeof(buf)) == 10);
	assert(memcmp(buf, "record-one", 10) == 0);
	assert(unixsock_dgram_recv(rx, buf, sizeof(buf)) == 10);
	assert(memcmp(buf, "record-two", 10) == 0);
	assert(close(tx) == 0);
	assert(unixsock_close_unlink(rx, path) == 0);
...
void	test_recv_on_empty_socket_returns_eagain(void)
{
	...
	rx = unixsock_dgram_bind(path, 0600);
	assert(rx >= 0);
	assert(unixsock_dgram_recv(rx, buf, sizeof(buf)) == -1);
...
void	test_open_on_missing_path_fails(void)
{
	...
	assert(unixsock_dgram_open(path) == -1);
	assert(errno == ENOENT);
...
void	test_send_without_receiver_is_a_reported_drop(void)
{
	...
	assert(tx >= 0);
	assert(close(rx) == 0);
	assert(unixsock_dgram_send_nonblock(tx, "orphan", 6) == -1);
...
void	test_rebind_over_stale_socket_file(void)
{
	...
	tmp_path(path, sizeof(path), "logd.sock");
	first = unixsock_dgram_bind(path, 0600);
	assert(first >= 0);
	assert(close(first) == 0);
	assert(access(path, F_OK) == 0);
	second = unixsock_dgram_bind(path, 0600);
	assert(second >= 0);
	assert(unixsock_close_unlink(second, path) == 0);
...
void	test_oversized_datagram_is_truncated(void)
{
	...
	assert(unixsock_dgram_send_nonblock(tx, big, sizeof(big)) == 0);
	assert(unixsock_dgram_recv(rx, small, sizeof(small)) == (ssize_t)sizeof(small));
	assert(unixsock_dgram_recv(rx, small, sizeof(small)) == -1);
	assert(close(tx) == 0);
	assert(unixsock_close_unlink(rx, path) == 0);
