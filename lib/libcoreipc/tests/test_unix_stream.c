...
void	test_listen_applies_mode(void)
{
	...
	fd = us_stream_listen(path, 4, 0600);
	assert(fd >= 0);
...
void	test_connect_to_missing_path_fails(void)
{
	...
	assert(us_stream_connect(path) == -1);
	assert(errno == ENOENT);
...
void	test_recv_all_reports_early_close(void)
{
	...
	conn = us_stream_accept(listen_fd);
	assert(conn >= 0);
	assert(us_recv_all(conn, got, sizeof(got)) == -1);
	assert(errno == EPIPE);
	assert(pthread_join(client, NULL) == 0);
	assert(close(conn) == 0);
	assert(us_close_unlink(listen_fd, path) == 0);
...
void	test_send_all_to_closed_peer_fails(void)
{
	...
	us_send_all(conn, payload, sizeof(payload));
	assert(us_send_all(conn, payload, sizeof(payload)) == -1);
...
static void	*client_sends_big(void *arg)
{
	...
	fd = us_stream_connect((const char *)arg);
	assert(fd >= 0);
	payload = malloc(BIG_LEN);
	assert(payload != NULL);
	memset(payload, 'A', BIG_LEN);
	assert(us_send_all(fd, payload, BIG_LEN) == 0);
	free(payload);
	assert(close(fd) == 0);
	return (NULL);
}
...
static void	*client_closes_early(void *arg)
{
	...
	fd = us_stream_connect((const char *)arg);
	assert(fd >= 0);
	assert(us_send_all(fd, "half", 4) == 0);
	assert(close(fd) == 0);
	return (NULL);
}
