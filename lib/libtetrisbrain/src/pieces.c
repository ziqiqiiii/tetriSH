...
/**
 * @brief Produces a freshly spawned piece of the given type.
 *
 * Returns the canonical spawn position and rotation for the tetromino,
 * centred near the top of the board.
 *
 * @param type The tetromino type to spawn.
 * @return A piece_t at its spawn position in rotation state 0.
 */
...
/**
 * @brief Tests whether a piece occupies only empty, in-bounds cells.
 *
 * Rejects pieces whose type or rotation falls outside the shape tables, then
 * checks each of the four occupied cells. Out-of-bounds cells read as filled
 * walls, so collisions with walls and floor are caught here too.
 *
 * @param b Pointer to the board to test against.
 * @param p Pointer to the piece to validate.
 * @return true if every cell of the piece is empty and in bounds, else false.
 */
...
/**
 * @brief Rotates a piece in place, applying SRS wall kicks.
 *
 * Computes the target rotation state, then tries each wall-kick offset for the
 * relevant transition in order until one yields a valid placement. The O piece
 * is rotation-invariant and only has its rotation relabelled. On success the
 * piece is updated; on failure it is left untouched. Adding 3 (instead of
 * subtracting 1) keeps the result non-negative before the % 4.
 *
 * @param b Pointer to the board to test placements against.
 * @param p Pointer to the piece to rotate (modified on success).
 * @param dir Rotation direction: +1 rotates clockwise, -1 counter-clockwise.
 * @return BRAIN_OK if a valid rotation was found, BRAIN_BLOCKED otherwise.
 */
...
/**
 * @brief Attempts to translate a piece by a column/row delta.
 *
 * Builds the moved piece and commits it only if the new position is valid.
 *
 * @param b Pointer to the board to test against.
 * @param p Pointer to the piece to move (modified on success).
 * @param dcol Column delta to apply.
 * @param drow Row delta to apply.
 * @return BRAIN_OK if the move was applied, BRAIN_BLOCKED if it was rejected.
 */
...
/**
 * @brief Locks a piece into the board, marking its cells filled.
 *
 * Writes a CELL_FILLED cell tagged with the piece's type/colour at each of the
 * four occupied positions. Pieces with an out-of-range type or rotation are
 * ignored.
 *
 * @param b Pointer to the board to stamp into.
 * @param p Pointer to the piece to lock.
 */
...
/**
 * @brief Reports whether a piece's type and rotation index the shape tables.
 *
 * A corrupted piece_t (e.g. from a malformed network message) could carry a
 * type or rotation outside the SHAPES/KICK tables; this guards every entry
 * point that indexes those tables by p->type/p->rotation.
 *
 * @param p Pointer to the piece to range-check.
 * @return true if both type and rotation are within range, false otherwise.
 */
static bool	piece_shape_in_range(const piece_t *p)
{
