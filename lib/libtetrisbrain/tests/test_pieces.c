	...
	for (i = 0; i < 7; i++)
	{
		...
		for (r = 0; r < BOARD_HEIGHT; r++)
			for (c = 0; c < BOARD_WIDTH; c++)
				if (board_get(&b, c, r).type == CELL_FILLED)
					filled++;
		assert(filled == 4);
	}
...
void	test_move_blocked_by_walls_and_floor(void)
{
	...
	board_init(&b);
	p = piece_spawn(PIECE_I); // col=3,row=-1, occupies cols 3-6, row 0
	for (i = 0; i < 3; i++)
...
void	test_rotate_basic_updates_rotation(void)
{
	...
	board_init(&b);
	p = piece_spawn(PIECE_T); // col=3,row=0,rotation=0
...
void	test_o_piece_rotate_is_noop(void)
{
	...
	board_init(&b);
	p = piece_spawn(PIECE_O); // col=4,row=0,rotation=0
	assert(piece_rotate(&b, &p, 1) == BRAIN_OK);
	assert(p.rotation == 1);
...
void	test_invalid_type_rejected_safely(void)
{
	...
	board_init(&b);
	p = (t_piece){(t_piece_type)99, 3, 0, 0}; // corrupted type, out of SHAPES range
	assert(!piece_is_valid(&b, &p));
...
void	test_invalid_rotation_rejected_safely(void)
{
	...
	board_init(&b);
	p = (t_piece){PIECE_T, 3, 0, 99}; // corrupted rotation, out of SHAPES range
	assert(!piece_is_valid(&b, &p));
	piece_stamp(&b, &p); // must not read OOB / must not touch the board
	for (r = 0; r < BOARD_HEIGHT; r++)
		for (c = 0; c < BOARD_WIDTH; c++)
