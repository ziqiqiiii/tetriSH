...
# a kernel-assigned port, throwaway certificates and a throwaway data
# directory, talked to over a real socket by tests/bin/net_smoke.
#
# tetrisd needs epoll and timerfd, so this is a Linux suite. Everywhere else it
# reports that it was skipped rather than failing — a client that cannot be
# exercised is not the same thing as a client that is broken.

set -uo pipefail

ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)
SMOKE="$ROOT/src/tetrisu/tests/bin/net_smoke"
DAEMON="$ROOT/src/tetrisd/tetrisd"
PORT=${TETRISU_TEST_PORT:-42421}
WORK=""
PID=""
...
WORK=$(mktemp -d "${TMPDIR:-/tmp}/tetrisu-net.XXXXXX")
